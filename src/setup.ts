...
export const setup = (p: p5): void => {
  ...
  const ascii = new Range(0x20, 0xff);
  const cjkUnifiedIdeographs = new Range(0x4e00, 0x9fff);
  const hiragana = new Range(0x3040, 0x3096);
  const katakana = new Range(0x30A1, 0x30FF);
  const target = katakana;

  let writer = p.createWriter("katakana.csv")
  for (let codePoint = target.from; codePoint <= target.to; codePoint++) {
...
export const getCharDensity = (p: p5, char: String): [number, number[], boolean] => {
